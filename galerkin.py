...
class Chebyshev(FunctionSpace):
    ...
    def L2_norm_sq(self, N):
        #return [np.pi if i == 0 else np.pi/2 for i in range(N)]
        a = np.zeros(N)
        a[0] = np.pi
        a[1:] = np.pi/2
        return a
...
class Cosines(Trigonometric):
    ...
    def L2_norm_sq(self, N):
        #return [1 if i == 0 else 0.5 for i in range(N)]
        a = np.zeros(N)
        a[0] = 1
        a[1:] = 0.5
        return a
...
class NeumannLegendre(Composite, Legendre):
    def __init__(self, N, domain=(-1, 1), bc=(0, 0), constraint=0):
        ...
        self.B = Neumann(bc, domain, self.reference_domain)
        d1 = np.ones(N + 1)
        d2 = [-j*(j + 1)/((j + 2)*(j + 3)) for j in range(N + 1)]
        self.S = sparse.diags((d1, d2), (0, 2), shape=(N+1, N+3), format='csr')

    def basis_function(self, j, sympy=False):
        a = j*(j + 1)/((j + 2)*(j + 3))
        if sympy:
            return sp.legendre(j, x) - a*sp.legendre(j + 2, x)
...
class NeumannChebyshev(Composite, Chebyshev):
    def __init__(self, N, domain=(-1, 1), bc=(0, 0), constraint=0):
        ...
        self.B = Neumann(bc, domain, self.reference_domain)
        d1 = np.ones(N+1)
        d2 = [-(j/(j + 2))**2 for j in range(N+1)]
        self.S = sparse.diags((d1, d2), (0, 2), shape=(N+1, N+3), format='csr')

    def basis_function(self, j, sympy=False):
        a = (j/(j + 2))**2
        if sympy:
            return sp.cos(j*sp.acos(x)) - a*sp.cos((j+2)*sp.acos(x))
        return Cheb.basis(j) - a*Cheb.basis(j+2)
